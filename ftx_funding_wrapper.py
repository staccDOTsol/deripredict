...
class DeribitWrapper(object):
    ...
    def fetch_chart_data(self, instrument="BTC-PERPETUAL", interval=1, days_lookback=60):
        try:
            ...
            print(start, end)
            rates = requests.get("https://ftx.com/api/funding_rates?limit=1000&future=" + instrument + '-PERP').json()['result']
            result = {}
            for rate in rates:
                coin = rate['future'].replace('-PERP','')
                ...
                if  'volume' not in result:
                    ...
                    result['close'] = []
                    result['cost'] = []
                result['volume'].append(rate['rate'])
                result['ticks'].append(rate['rate'])
                result['open'].append(rate['rate'])
                result['low'].append(rate['rate'])
                result['high'].append(rate['rate'])
                result['close'].append(rate['rate'])
                result['cost'].append(rate['rate'])
